    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsMarkup = participants.length
          ? `<div class="participants-list">${participants
              .map(
                (participant) => `
                  <span class="participant-item" data-email="${participant}">
      ...
      document.querySelectorAll(".participant-remove").forEach((button) => {
        button.addEventListener("click", async () => {
          const participantChip = button.closest(".participant-item");
          const email = participantChip?.dataset.email;
          const activityName = participantChip?.closest(".activity-card")?.querySelector("h4")?.textContent;
